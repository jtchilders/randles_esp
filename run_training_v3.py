...
def wrapped_loader(path):
   ...
   # want to target a size of 10k for mout and 100k for pv_data
   
   # sub sample
   mout = random_subsample(mout,1e4)
   pv_data = random_subsample(pv_data,1e5)
...
def main():
   ...
   print("GPUS: ",tf.config.experimental.get_visible_devices('GPU'))

   data_path = '/lus/eagle/projects/multiphysics_aesp/data/FullCSVs/'
   filelist = glob.glob(data_path + '/00cts?l*combined_csv*.csv')
   if hvd.rank() == 0:
   ...
   for i_epoch in range(10):
      ...
      for step,(mx,pv) in enumerate(testing_ds):
         ...
         dur = end - start
         if(hvd.rank() == 0):
            print("[%05d] time = %s loss = %f  loss_mean: %f  acc_mean: %f  points: %d %d" % (i_epoch*total_steps + step,str(dur),loss,test_loss.result(),test_acc.result(),pv.get_shape().as_list()[0],mx.get_shape().as_list()[0]))
            sys.stdout.flush()
